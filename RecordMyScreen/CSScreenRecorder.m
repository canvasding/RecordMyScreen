...
- (void)_setupVideoContext
{
    ...
    if ([[UIDevice currentDevice] userInterfaceIdiom] == UIUserInterfaceIdiomPhone) {
        ...
        if ([modelName isEqualToString:@"iPad2,5"]) { //iPad mini
            // iPad Mini frame buffer is Portrait
            _width = screenRect.size.width * scale;
            _height = screenRect.size.height * scale;
        }
        
        if ([modelName isEqualToString:@"iPad2,6"]) { //iPad mini
            // iPad Mini frame buffer is Portrait
            _width = screenRect.size.width * scale;
            _height = screenRect.size.height * scale;
        }
        
        if ([modelName isEqualToString:@"iPad2,7"]) { //iPad mini
            // iPad Mini frame buffer is Portrait
            _width = screenRect.size.width * scale;
            _height = screenRect.size.height * scale;
        }
